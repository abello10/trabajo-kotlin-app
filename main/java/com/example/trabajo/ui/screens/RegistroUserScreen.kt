        ...
        Button(
            onClick = {
                if (viewModel.validarFormulario()) {
                    viewModel.registrarUsuario { exito ->
                        if (exito) {
                            // Si el backend respondió OK, guardamos en DataStore y navegamos
                            scope.launch {
                                setLoggedIn(ctx, true)
                                setUser(ctx, estado.name, estado.correo, estado.contrasena)
                                navController.navigate("games") { popUpTo(0) }
                            }
                        } else {
                            // Aquí podrías mostrar un mensaje de error (Snackbar/Toast)
